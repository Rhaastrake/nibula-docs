...
```
src/backend/
├── core/            the engine, you don't touch this
├── endpoints/
...
You work in **endpoints**. A file's path is its URL: `endpoints/public/contact.js` answers at `/api/contact`, and subfolders work the same way.

**core** receives every request, checks the API key when needed, and calls your file. Nothing to configure.
...
An endpoint is one file. Inside it you write one function per HTTP method you accept: anything else gets an automatic **405**, so you never check the method yourself.

```js
module.exports = {
  get: ({ query, Response }) => {
    Response.success({ message: 'It works', query });
  },
...
### Checking what arrives {id="backend-validate"}

Never trust what a client sends. `Validate` comes with the request and stops it by itself when a value is wrong, so you don't need an `if`:

| Check | What it wants |
|---|---|
| `required` | Anything, as long as it isn't empty |
| `integer` | A number, given back to you as a number |
| `email` | An address that looks like one |
| `minLength` | At least that many characters |

The second argument is the field name, and it's what the client reads in the error message. A failed check answers with **422**.
...
**PHP** uses Composer, from `src/backend/core`:
