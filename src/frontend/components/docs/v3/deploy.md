...
Make sure `config.js` exists in `out/backend/`. If it doesn't, copy `example.config.js` to `config.js` and fill in your values. The backend won't start without it.
...
ExecStart=/usr/bin/node /var/www/SITE_FOLDER/backend/core/index.js
Restart=always
RestartSec=5
